...
func AssertCreateTestDataLargeObject(namespace, clusterName string, oid int, data string, pod *corev1.Pod) {
	By("creating large object", func() {
		...
		superUserPass, err := testsUtils.GetPassword(clusterName, namespace, apiv1.SuperUserSecretSuffix, env)
		Expect(err).ToNot(HaveOccurred())
...
func insertRecordIntoTableWithDatabaseName(
	...
	pass, err := testsUtils.GetPassword(clusterName, namespace, apiv1.SuperUserSecretSuffix, env)
	Expect(err).ToNot(HaveOccurred())
	host, err := testsUtils.GetHostName(namespace, clusterName, env)
	Expect(err).ToNot(HaveOccurred())
	_, _, err = testsUtils.RunQueryFromPod(
...
func AssertReplicaModeCluster(
	...
	By("creating test data in source cluster", func() {
		...
		superUserPass, err := testsUtils.GetPassword(srcClusterName, namespace, apiv1.SuperUserSecretSuffix, env)
		Expect(err).ToNot(HaveOccurred())
...
func AssertClusterAsyncReplica(namespace, sourceClusterFile, restoreClusterFile, tableName string, pod *corev1.Pod) {
	By("Async Replication into external cluster", func() {
		...
		superUserPass, err := testsUtils.GetPassword(sourceClusterName, namespace, apiv1.SuperUserSecretSuffix, env)
		Expect(err).ToNot(HaveOccurred())
		...
		out, _, err := testsUtils.RunQueryFromPod(
			psqlClientPod, rwService, testsUtils.AppDBName, testsUtils.PostgresUser, superUserPass, query, env)
...
func assertReadWriteConnectionUsingPgBouncerService(
	...
	generatedAppUserPassword, err := testsUtils.GetPassword(clusterName, namespace, apiv1.ApplicationUserSecretSuffix, env)
	Expect(err).ToNot(HaveOccurred())
